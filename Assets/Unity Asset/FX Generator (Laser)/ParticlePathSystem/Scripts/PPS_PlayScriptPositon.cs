...
{
	public class PPS_PlayScriptPositon : PPS_PathPositions
	{
		...
		void OnEnable()
		{
			GetPositions();
			if (autoPlay)
				if (GetComponent<ParticlePath>()) GetComponent<ParticlePath>().InitializePath(positions, speed);
		}
		public override List<Vector3> GetPositions()
		{
			...
			else if (type == DrawType.ring)
			{
				...
				for (float i = 20; i >= -20; i -= 0.1f)
				{
					...
					float y = Mathf.Sin(angle * index * Mathf.Deg2Rad) * 2;
					float z = Mathf.Cos(angle * index * Mathf.Deg2Rad) * 2;
